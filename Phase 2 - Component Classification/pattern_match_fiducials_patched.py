...
parser.add_argument('--imageFilepath', help="The filepath to the input image. Default: './images/board0_pass_nodefects_6.jpg'", default='./images/board0_pass_nodefects_6.png')
...
parser.add_argument('--fiducialInnerDiameterInPixels', help="The diameter of the fiducials' inner bright disk. Default: 26", type=int, default=18)
parser.add_argument('--fiducialOuterDiameterInPixels', help="The diameter of the fiducials' outer dark disk. Default: 68", type=int, default=48)
parser.add_argument('--numberOfFiducials', help="The number of fiducials. Default: 3", type=int, default=8)
...
def main():
    ...
    for (seed_point, bounding_box) in optimal_seedPoint_boundingBox_list:
        ...
        # Assign known fiducial positions
        if blob_center[0] > img_shapeHWC[1] // 2 and blob_center[1] < img_shapeHWC[0] // 2:
            ...
            # Fallback: assign to whichever slot isn't filled yet
            if 'NorthEast' not in quadrant_to_pixels_location_dict:
                quadrant_to_pixels_location_dict['NorthEast'] = blob_center
            elif 'SouthWest' not in quadrant_to_pixels_location_dict:
                quadrant_to_pixels_location_dict['SouthWest'] = blob_center
            else:
                logging.warning(f"Extra blob at {blob_center} ignored.")

        # Draw rectangle and circle
        cv2.rectangle(
            annotated_img,
            (blob_center[0] - args.fiducialOuterDiameterInPixels // 2,
            blob_center[1] - args.fiducialOuterDiameterInPixels // 2),
            (blob_center[0] + args.fiducialOuterDiameterInPixels // 2,
            blob_center[1] + args.fiducialOuterDiameterInPixels // 2),
            color=(0, 255, 0),
            thickness=3
        ...
        cv2.circle(
            annotated_img,
            blob_center,
            args.fiducialInnerDiameterInPixels // 2 + 2,
            (0, 0, 255),
            thickness=-1
        )
    ...
    synthetic_center = (
    (quadrant_to_pixels_location_dict['NorthEast'][0] + quadrant_to_pixels_location_dict['SouthWest'][0]) // 2,
    (quadrant_to_pixels_location_dict['NorthEast'][1] + quadrant_to_pixels_location_dict['SouthWest'][1]) // 2
)
    synthetic_center = (
        (quadrant_to_pixels_location_dict['NorthEast'][0] + quadrant_to_pixels_location_dict['SouthWest'][0]) // 2,
        (quadrant_to_pixels_location_dict['NorthEast'][1] + quadrant_to_pixels_location_dict['SouthWest'][1]) // 2
    ...
    position_mm_to_pixels_dict = {
        ( 22.4, -20.4): quadrant_to_pixels_location_dict['NorthEast'],
        (-18.4,  30.0): quadrant_to_pixels_location_dict['SouthWest'],
        (  0.0,   0.0): synthetic_center
    }
    ...
    corner_to_pixels = {}
    corner_to_pixels['NorthWest'] = transformation.Transform(mm_to_pixels_transformation_mtx, (-30.0, -30.0))
    corner_to_pixels['NorthEast'] = transformation.Transform(mm_to_pixels_transformation_mtx, (30.0, -30.0))
    corner_to_pixels['SouthEast'] = transformation.Transform(mm_to_pixels_transformation_mtx, (30.0, 30.0))
    corner_to_pixels['SouthWest'] = transformation.Transform(mm_to_pixels_transformation_mtx, (-30.0, 30.0))
    ...
    cv2.imwrite(os.path.join(args.outputDirectory, "grayscale4.png"), grayscale_img)
    cv2.imwrite(os.path.join(args.outputDirectory, "fiducialPattern4.png"), fiducial_pattern)
    cv2.imwrite(os.path.join(args.outputDirectory, "match4.png"), padded_match_8bits_img)
    cv2.imwrite(os.path.join(args.outputDirectory, "blobs4.png"), optimal_annotated_blobs_img)
    cv2.imwrite(os.path.join(args.outputDirectory, "foundFiducials4.png"), annotated_img)
